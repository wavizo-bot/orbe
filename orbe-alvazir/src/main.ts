...
async function gerarContraSenhas(senha: string) {
  ...
  const senhaExibida = document.getElementById("senha-exibida") as HTMLInputElement;
  ...
  for (const tipo of tipos) {
    const contraSenha =
      todasContraSenhas[tipo.chave as "1m" | "3m" | "6m" | "12m"];
...
async function inicializar() {
  ...
  document.getElementById("btn-colar")!.addEventListener("click", async () => {
    ...
    (document.getElementById("input-senha") as HTMLInputElement).value =
  ...
  document
    ...
    .addEventListener("keypress", async (e) => {
      if (e.key === "Enter") {
        const senha = (e.target as HTMLInputElement).value.toUpperCase();
        if (senha.length === 8) {
          await gerarContraSenhas(senha);
        }
      }
