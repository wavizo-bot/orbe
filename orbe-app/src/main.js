import { db } from "./db";
import { gerarSenhaDoDia, gerarTodasContraSenhas, validarSenhaTeste, somarMeses, formatarDataExibicao, calcularIdade, } from "@orbe/algoritmo-senha";
import { NUMERO_SUPORTE, CHAVE_DEVICE_ID } from "@orbe/constantes";
...
let historicoAlunos = [];
let conferenciaSelecionada = new Set();
let imagensInstitucionais = new Map();
// ==== Utilidades ====
function gerarOuCarregarDeviceId() {
...
function abrirModal(titulo, corpo, onConfirmar = () => { }, textoConfirmar = "Confirmar") {
...
async function inicializarAutenticacao() {
    ...
    // Exibe senha
    const exibicao = document.getElementById("auth-senha-exibida");
...
async function validarContraSenhaDigitada(contrassenhaDigitada) {
    ...
    document.getElementById("auth-contrassenha").value =
        "";
}
// ==== Menu Principal ====
...
// ==== Alunos ====
async function renderizarAlunos(lista) {
...
function abrirPerfilAluno(aluno) {
    ...
    db.fotos.get(aluno.IDunico).then((fotoObj) => {
        ...
        if (aluno.dn) {
            ...
            const p = document.createElement("p");
            p.textContent = `Data nascimento: ${formatarDataExibicao(aluno.dn)} (${idade} anos)`;
            secao.appendChild(p);
        }
...
function atualizarBotaoMostrar() {
    const alunosFiltr = alunos.filter((a) => (filtrosSelecionados.salas.size === 0 ||
...
async function abrirConferencia(lista) {
    ...
    lista.sort((a, b) => {
        ...
        const ordem = { verde: 0, amarelo: 1, padrao: 2 };
...
async function inicializar() {
    ...
    document.getElementById("btn-limpar-busca").addEventListener("click", () => {
        document.getElementById("input-busca-alunos").value =
            "";
        buscarAlunos("");
    });
    // Botões de menu
    ...
    const historicoNavegacao = ['menu'];
    // Atualiza histórico quando muda de tela
    const trocarTelaOriginal = trocarTela;
    window.trocarTelaComHistorico = function (novaTela) {
    ...
    function adicionarAoHistoricoBrowser(tela) {
    ...
    document
        ...
        .addEventListener("click", async () => {
        const contrassenha = document.getElementById("auth-contrassenha").value;
